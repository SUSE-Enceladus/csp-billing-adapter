The csp-billing-adapter requires a configuration file, (defaults to `~/.config/csp_billing_adapter/config.yaml`)
in order to run successfully. The configuration files in in the `examples` directory 
...
query_interval:
    Time unit how often the csp-billng-adapter collectd the data from the application.
...
reporting_interval: 
    Sets the time when csp-billing-adapter reports to the CSP API. The values the csp-billing-adapter reports are determined by a combination of the settings for `reporting_api_is_cumulative` and `billing_interval`.
  ...
  {metric name}:
    ...
    consumption_reporting:
        ...
        The volume value triggers the code to search for the highest possible dimension to use and all other configured dimensions will be reported as 0, or not reported, depending on CSP API requirements. For example:
            ...
            In this case the application API may report only 1 dimension. If the application API reports more than 1 dimension it is an error condition.
        ...
        The tiered value triggers the code to report per dimension. This has 2 implied reporting mechanisms.
            ...
            One use case is the application API reports dimensions that match the configuration.
                ...
                In this case, the csp-billing-adapter reports the count per dimension as obtained by the application API.
            ...
            The second use case is for the application API to return only 1 value
                ...
                - A minimum setting of 5, 31, and 41
    ...
    usage_aggregation:
        The value the csp-billing-adapter reports. The maximum over the reporting time period, the current value, or the average over the reporting time period:
            ...
            average
                When set to this value, the billing adapter queries the application API hourly and stores the retrieved information in he internal data structure. When the billing adapter reports to the CSP API the average over all values is calculated and this is the number of units being reported. The internal storage structure is cleared. Expressed differently sum_of_all_numbers_in_array / length_of_array
    ...
    dimensions:
        A copy of the structure the csp-billing-adapter obtained from the application with the values reported at the time when the `last_billed` timestamp was set. This information is static during the time the csp-billing-adapter reports zeros to the CSP API. A list of 
